...
// The compare function; will change based on the implementation
...
void update(int idx, int val) {
	st[idx += n] = val;
	for (idx /= 2; idx; idx /= 2)
		st[idx] = comp(st[2 * idx], st[2 * idx + 1]);
}
...
// DFS to find the subtree count,
// The child with the most value will be a heavy child
...
// The Heavy light decompostion starts
...
int path(int x, int y){
	int ret = 0;  //<< Don't forget to pre initialize the starting value
	while(tp[x] != tp[y]){
		if(dep[tp[x]] < dep[tp[y]])swap(x,y);
		ret = comp(ret, query(id[tp[x]],id[x]));
		x = p[tp[x]];
	}
	if(dep[x] > dep[y])swap(x,y);
	ret = comp(ret, query(id[x],id[y]));
	return ret;
}

int getAns(int x,int y,LCA &lca){
    int l = lca.findLca(x,y);
    return comp( path(x,l), path(y,l) );

}

int main() {
    comp = getMax;  //< The comparator function is assigned to the pointer
	scanf("%d%d", &n, &q);
	LCA lca(n+5);
	...
	while(q--) {
		...
		if(t == 1) {
			...
			int a, b;
			scanf("%d%d", &a, &b);
			int res = getAns(a,b,lca);   //<Here we divide the path into 2 secgment. From a to LCA(a,b) and then to b to LCA(a,b) and merge the result
			printf("%d\n", res);
		}
	}
}
