...
void solveEulerTour(){
    ...
    for(int i=1;i<=n;i++){
        if( G[i].size()%2 ){ // each vertex must have even degrees
...
void solveEulerPath(){
    ...
    for(int i=2;i<n;i++){
        if( G[i].size()%2 ){ // each intermediate vertex must have even degrees
            cout<<"IMPOSSIBLE"<<endl;
            return ;
        }
    }
    if( G[1].size()%2!=1 and G[n].size()%2!=1 ){
        // start and end vertex must have odd degrees
        cout<<"IMPOSSIBLE"<<endl;
        return ;
    }
    etour(1);
