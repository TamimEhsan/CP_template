...
/* A string of length n can have maximum of n unique palindromes.
    so, palindromic tree can have maximum n(+2) nodes.
*/
struct PalindromicTree {
    struct node {
        int nxt[26], len, st, en, link, diff, slink, cnt, oc;
        /*  st is the start point of the palindromic substring
            en is the end point of the palindromic substring
                so, s.substr(st,en-st+1) will give the substring
            cnt the number of palindromic suffixes of the substring
            oc is the number of occurance of the substring
         */
    };
    string s;
    vector<node> t; //<each node represents a palindrome
    int sz, last;
    PalindromicTree() {}
...
int32_t main() {
    ...
    t.calc_occurrences();
    long long ans = 0;
    cout<<t.sz<<endl; //< 2 extra nodes were added, and number of nodes are the unique palindromes + 2
    for (int i = 3; i <= t.sz; i++){  //< 1-indexed, and 2 extra , so starting from 3 U w U
         ans += t.t[i].oc;
         //cout<<i<<" "<<t.t[i].oc<<" "<<s.substr(t.t[i].st,t.t[i].en-t.t[i].st+1)<<" "<<t.t[i].cnt<<endl;
    }
    cout << ans << '\n';
    return 0;
